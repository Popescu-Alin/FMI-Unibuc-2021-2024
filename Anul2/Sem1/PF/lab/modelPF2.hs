...
-- functie "sirulnr1" "sirdoi" = “sir”

functie :: String -> String -> String
functie sir1 "" = ""
functie "" sir2 = ""
functie (x:xs) (y:ys) = if x == y then [x] ++ functie xs ys else ""
...
fct x [] = []
fct [] y = []
fct (x:xs) (y:ys) = x^2 * y^2  : fct xs ys
...
fromPToExp:: PairInt-> Exp
fromPToExp (P x y) = Add (I x) (I y)
instance MyClass MyList where 
    toExp (L []) = I 1
    toExp (L x) = Mul (fromPToExp (head x)) (toExp (L (tail x))) 
...
fromPtToInt (P x y) = x+y
eval :: MyList -> Int
eval (L []) = 1
eval (L x) =  fromPtToInt (head x) * eval (L (tail x))
